...
def view_dashboard(request: Request, page: int = 1, db: Session = Depends(get_db)):
    ...
    urls = db.query(models.URL).order_by(models.URL.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
...
def shorten_url(
    ...
    if custom_alias:
        if db.query(models.URL).filter(models.URL.custom_alias == custom_alias).first() or \
           db.query(models.URL).filter(models.URL.short_code == custom_alias).first():
            return HTMLResponse("<div class='text-red-500 text-sm mt-2 font-bold'>Error: Alias already taken.</div>", status_code=400)
...
def redirect_to_target(short_code: str, request: Request, db: Session = Depends(get_db)):
    ...
    if not db_url:
        db_url = db.query(models.URL).filter(models.URL.custom_alias == short_code).first()
    ...
    if not db_url.is_active:
        return HTMLResponse("<h1 style='text-align:center;font-family:sans-serif;margin-top:50px;'>This link has been disabled.</h1>", status_code=410)
        
    if db_url.expires_at and db_url.expires_at < datetime.now():
        return HTMLResponse("<h1 style='text-align:center;font-family:sans-serif;margin-top:50px;'>This link has expired.</h1>", status_code=410)

    if db_url.hashed_password:
        return templates.TemplateResponse(request=request, name="password_prompt.html", context={"short_code": short_code})
...
def verify_password_and_redirect(short_code: str, request: Request, password: str = Form(...), db: Session = Depends(get_db)):
    ...
    if not pwd_context.verify(password, db_url.hashed_password):
        return templates.TemplateResponse(request=request, name="password_prompt.html", context={"short_code": short_code, "error": "Incorrect password"})
